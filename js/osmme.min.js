...
function getWHTable(e,a,h){var d=[],b="MO TU WE TH FR SA SU".split(" "),g;for(g in b){var f=[];d.push(f);var k=a.value[b[g]];f.push({text:e.dayNames[g]});k?k[2]?f.push({text:k[0]+"-"+k[k.length-1]+"<br>"+k[1]+"-"+k[2]}):f.push({text:k[0]+"-"+k[1]}):f.push({text:h.tr(e,"details.poi.tag.values.wh.off")})}return d}
...
OSMmeApp.controller("MapController",["$rootScope","$scope","$cookies","i18nService","mapService","search","docTree","details","iGeocoder","$location","routeService","Analytics",function(e,a,h,d,b,g,f,k,r,m,l,p){function u(){var b=l.getParameters(),d=[],g=[];b.pg&&b.pg.split(",").forEach(function(a){a&&d.push(a)});b.pt&&b.pt.split(",").forEach(function(a){a&&g.push(a)});d=filterAndSort(d);g=filterAndSort(g);!a.osmdocCat||angular.equals(d,a.osmdocCat.groups)&&angular.equals(g,a.osmdocCat.features)||
(a.osmdocCat.groups=d,a.osmdocCat.features=g,f.organizeCathegories(),f.updateSelections(a),f.loadTagValuesStatistic(a),a.$broadcast("SelectCathegoryTreeNode"))}a.SITE_SESSION=Math.floor(65536*(1+Math.random())).toString(16).substring(1);a.HTML_ROOT=HTML_ROOT;e.HTML_ROOT=HTML_ROOT;e.pathWithRoot=function(a){return HTML_ROOT+a};a.pathWithRoot=function(a){return HTML_ROOT+a};e=m.search();l.flag("m");l.anonymous("lang");l.parameter("id");l.parameter("map",3,!0);l.parameter("pg");l.parameter("pt");l.parameter("q");
l.flag("strict");l.flag("details");l.flag("explain");if(e.fid){var q={};q.id=e.fid;e.details&&(q.details=!0);m.search("");m.path("");l.update(q)}a.searchForm={};a.langsAvaible=["en","ru"];var n=l.getParameters();0>a.langsAvaible.indexOf(n.lang)&&(m=initLang(h),0<=a.langsAvaible.indexOf(n.lang)?l.update("lang",m):l.update("lang",DEFAULT_LANGUAGE));a.mobile=isMobile();n.m!=a.mobile&&l.update("m",a.mobile);ANALYTICS_CODE&&p.trackPage("/#!/"+a.lng+"/");angular.element(document.getElementsByTagName("head")[0]).append('<link rel="stylesheet" type="text/css" href="css/'+
(a.mobile?"mobile":"descktop")+'.css" />');a.lng=n.lang||initLang(h);a.hierarchyCode=f.getHierarchyCode(a.lng);a.name2FClass={};a.name2Group={};f.loadTree(a,a.lng,a.hierarchyCode);d.getTranslation(a,a.lng,!0,function(){var l=[null,null,null];n.map&&(l=n.map);var k=b.createMap(a,l[1],l[2],l[0]);r.create(a,k);f.attach(a);g.attach(a);a.dayNames=d.tr(a,"details.poi.tag.values.wh.days").split(" ");b.enquirePosition(function(b){if(b){var d=b.coords.latitude,f=b.coords.longitude;500<k.getCenter().distanceTo([d,
f])&&r.sendRequest(f,d,!1,function(b){b&&b.text?(b.lat=d,b.lon=f,a.browserGeoLocation=b):a.browserGeoLocation&&delete a.browserGeoLocation})}})});a.activeFeatureID=n.fid;a.explain=n.explain;a.content=n.details?"details":"map";a.activeFeatureID&&("details"==a.content?k.showDetails(a,a.activeFeatureID):k.showPopup(a,a.activeFeatureID));a.switchPoiClassArray=function(b){try{var d=a.osmdocCat.features.slice(0),f=a.osmdocCat.features.indexOf(b);0>f?d.push(b):d.splice(f,1);return filterAndSort(d)}catch(g){return[]}};
a.poiClassTranslatedTitle=function(b){return b.title_by_lang&&b.title_by_lang[a.lng]?b.title_by_lang[a.lng]:b.translated_title[0]};a.$on("$locationChangeSuccess",function(){var d=a.activeFeatureID,f=l.getParameters();a.lng!=f.lang&&(h.lang=f.lang,l.reload());a.activeFeatureID=f.id;a.explain=f.explain;a.content=f.details?"details":"map";"details"==a.content?(ANALYTICS_CODE&&p.trackPage("/#!/"+a.lng+"/id/"+a.activeFeatureID+"/details"),k.showDetails(a,a.activeFeatureID),d&&k.closePopup(a,d)):(a.activeFeatureID?
(ANALYTICS_CODE&&p.trackPage("/#!/"+a.lng+"/id/"+a.activeFeatureID),k.showPopup(a,a.activeFeatureID)):void 0!==d&&d!=a.activeFeatureID&&k.closePopup(a,d),l.getParameters().map&&(d=l.getParameters().map,b.broadcastAction||b.setView(d[1],d[2],d[0])));f.strict&&(a.strictSearch=!0);f.q&&null==a.searchForm.q&&a.searchForm.q!=f.q&&(a.searchForm.q=f.q,a.$broadcast("Search",f.q));u()});a.$on("Search",function(){l.update("q",a.searchForm.q)});a.$on("SelectFeature",function(){l.update("q",a.searchForm.q)});
a.$on("HierarchyLoaded",u);a.$on("PopupClose",function(b,d){"map"==a.content&&l.update("id",null)});a.$on("PopupOpen",function(b,d,f){l.update({id:d,map:null});f&&k.loadPTRoutes(a,d,f)});a.$on("MapViewChanged",function(){l.update("map",b.getStateArray())});m=function(){a.osmdocCat&&(f.organizeCathegories(),0<a.osmdocCat.features.length?l.update("pt",a.osmdocCat.features.join()):l.update("pt",null),0<a.osmdocCat.groups.length?l.update("pg",a.osmdocCat.groups.join()):l.update("pg",null),b.filterMarkersByTypes(a,
...
a.mergeIntoPath=function(a,b){return"/#!"+l.mergeIntoPath(a,b)};a.searchKeyDown=function(b){38==b.keyCode?(b.stopPropagation(),b.preventDefault(),a.$broadcast("SearchKeyUp")):40==b.keyCode&&a.$broadcast("SearchKeyDown")};a.$watch("searchForm.q",function(b,f){""!=f&&""==b&&a.$broadcast("CleanSearchInput")});a.$on("CleanSearchInput",function(){l.update("q",null)});a.navigate=function(a,b){l.update(a,b)};a.formatSearchResultTitle=function(b){if(!b)return"";if("adrpnt"==b.type)return d.tr(a,"map.js.search.title.adrpnt");
...
(this.parameters.push(b),this.byName[a]=b)};a.prototype.template=function(){var a="/";angular.forEach(this.parameters,function(d){"anonymous"==d.type&&(a+=":"+d.name+"/");"flag"==d.type&&(a+="?"+d.name+"/");if("parameter"==d.type&&(a+=d.name+"/:"+d.name+"/",d.slashes))for(var b=0;b<d.slashes-1;b++)a+=":"+d.name+"/"});return a};a.prototype.getParameters=function(){var a=[];angular.forEach(e.path().split("/"),function(b){b&&a.push(b)});for(var d=0,b={},g=0;g<a.length;g++)for(var f=d;f<this.parameters.length;f++){var k=
this.parameters[f],r=a[g];if("anonymous"==k.type){b[k.name]=r;d++;break}else if("flag"==k.type&&k.name==r){b[k.name]=!0;d++;break}else if("parameter"==k.type&&k.name==r){if(k.slashes)for(f=g+1;f<Math.min(a.length,g+1+k.slashes);f++)b[k.name]||(b[k.name]=[]),b[k.name].push(a[f]);else b[k.name]=a[g+1];g+=k.slashes||1;d++;break}d++}return b};a.prototype.createPath=function(a){for(var d="/",b=0;b<this.parameters.length;b++){var g=this.parameters[b];"anonymous"==g.type&&a[g.name]&&(d+=a[g.name]+"/");"flag"==
...
(function(e,a){function h(a,b){var g=Math.pow(10,b);return Math.round(a*g)/g}a.factory("mapService",["i18nService","$rootScope","$cookies","$compile","$http",function(a,b,g,f,k){var e=L.Control.extend({container:null,options:{position:"topleft"},setScope:function(a){this.scope=a;return this},setContainer:function(a){var b=L.DomUtil.create("div","search-control");L.DomEvent.disableClickPropagation(b);L.DomEvent.addListener(b,"mousewheel",function(a){var b=document.getElementById("r-scroll-pane");b&&
(b.scrollTop-=20*L.DomEvent.getWheelDelta(a));L.DomEvent.stopPropagation(a)});b.innerHTML=a;this.container=b},onAdd:function(a){return this.container}}),m={id2Feature:{},id2Marker:{},ready:!1,createMap:function(l,p,h,q){p=p||g.lat||DEFAULT_LAT;if(90<p||-90>p)p=DEFAULT_LAT;h=h||g.lon||DEFAULT_LON;if(180<h||-180>h)h=DEFAULT_LON;q=q||g.z||DEFAULT_ZOOM;0>=q&&(q=DEFAULT_ZOOM);this.map=L.map("map",{zoomControl:!1}).setView([p,h],q);p=a.tr(l,"map.js.copy.data")+' &copy; <a href="http://osm.org">'+a.tr(l,
"map.js.copy.contributors")+"</a>, "+a.tr(l,"map.js.copy.rendering")+' <a href="http://giscience.uni-hd.de/" target="_blank">University of Heidelberg</a>';l.mobile&&(p='<a href="http://osm.org">OpenStreetMap</a>,  <a href="http://giscience.uni-hd.de/" target="_blank">University of Heidelberg</a>');this.mapsurfer=L.tileLayer(MAPSURFER_TILES,{attribution:p,maxZoom:18});p=a.tr(l,"map.js.copy.contributors")+'&copy; <a href="http://osm.org/copyright">OpenStreetMap</a>';this.mapnik=L.tileLayer("http://{s}.tile.osm.org/{z}/{x}/{y}.png",
{attribution:p});p={};p[a.tr(l,"map.js.layer.relief")]=L.tileLayer("http://openmapsurfer.uni-hd.de/tiles/asterh/x={x}&y={y}&z={z}",{maxZoom:18});p[a.tr(l,"map.js.layer.contours")]=L.tileLayer("http://openmapsurfer.uni-hd.de/tiles/asterc/x={x}&y={y}&z={z}",{maxZoom:18});h={Mapnik:this.mapnik,MapSurfer:this.mapsurfer};(this[DEFAULT_LAYER||"mapsurfer"]||mapsurfer).addTo(this.map);l.mobile||(this.layersControl=L.control.layers(h,p),this.layersControl.addTo(this.map));var n=this.map;this.map.on("viewreset",
function(){window.setTimeout(function(){m.saveStateToCookie.apply(m,[]);m.broadcastAction=!0;l.$broadcast("MapViewChanged",n.getCenter(),n.getZoom());b.$$phase||b.$apply();m.broadcastAction=!1},10)});this.map.on("moveend",function(){window.setTimeout(function(){m.saveStateToCookie.apply(m,[]);m.broadcastAction=!0;l.$broadcast("MapViewChanged",n.getCenter(),n.getZoom());b.$$phase||b.$apply();m.broadcastAction=!1},10)});this.map.on("popupopen",function(a){var d=n.project(a.popup._latlng);d.y-=a.popup._container.clientHeight/
2;n.panTo(n.unproject(d),{animate:!1});d=a.popup._source?a.popup._source.id:a.popup.id;f(a.popup._contentNode)(l);l.$broadcast("PopupOpen",d,m.id2Feature[d].poi_class);b.$$phase||b.$apply()});this.map.on("popupclose",function(a){l.$broadcast("PopupClose",a.popup._source?a.popup._source.id:a.popup.id);b.$$phase||b.$apply()});this.ready=!0;if(!l.mobile){var x=this.searchControl=(new e).setScope(l);k.get(HTML_ROOT+"/templates/search_control.html").success(function(a){x.setContainer(a);
x.addTo(n);f(x.container)(l)})}return this.map},createPopUP:function(a,b){if(!this.id2Feature[b.id]){this.id2Feature[b.id]=b;var f=b.poi_class?a.name2FClass[b.poi_class[0]]:null,d=this;this.loadIcon(f,function(){var g=L.marker([b.centroid.lat,b.centroid.lon]);f&&f.ll_icon&&(g=L.marker([b.centroid.lat,b.centroid.lon],{icon:f.ll_icon}));d.id2Marker[b.id]=g;var k=d.getPopUPHtml(b,b.id,a,f);g.addTo(d.map).bindPopup(k);g.id=b.id})}},openPopUP:function(a,
b,f){if(b&&this.id2Marker[b])if(angular.element(this.map.getContainer()).hasClass("ng-hide")){var d=this,g=f||5;0<g&&window.setTimeout(function(){d.openPopUP.apply(d,[a,b,g])},100)}else this.id2Marker[b]._popup._isOpen||(this.map.invalidateSize(!1),this.id2Marker[b].openPopup())},closePopUP:function(a,b){b&&this.id2Marker[b]&&this.id2Marker[b].closePopup()},isPopUPExists:function(a,b){return b&&!!this.id2Marker[b]},getPopUPHtml:function(b,f,g,k){var e=g.formatSearchResultTitle(b);b=getAddress(b,g.translation?
g.translation["addr.order"]:"hn-street-city")[0];g='<a class="more-link" href="'+g.mergeIntoPath({details:!0,id:f})+'">'+a.tr(g,"map.js.popup.more")+"</a>";var h="";!k||"tram_stop"!=k.name&&"bus_stop"!=k.name||(h="<h5 ng-bind=\"translation['pt.routes']\"></h5><div ng-hide=\"getPTRoutes('"+f+'\') != null"><img ng-src="{{HTML_ROOT}}/img/loading.gif"/></div><div ng-show="getPTRoutes(\''+f+'\')" class="routes-list" ng-repeat="(type, refs) in getPTRoutes(\''+f+'\').routes"><span ng-bind="translation[\'pt.route.type.\' + type]"></span>&nbsp;<span class="route-ref" ng-repeat="ref in refs" ng-bind="ref"></span></div>');
...
h=a.searchResultsPage.size,m=parseInt(f/h);0==f%h&&(m+=1);for(f=1;f<=m&&8>=f;f++)d[f]={p:f,active:e==f};for(f=e-1;f<=e+1;f++)0<f&&f<=m&&(d[f]={p:f,active:e==f});for(f=m-2;f<=m;f++)0<f&&(d[f]={p:f,active:e==f})}var l=[];angular.forEach(d,function(a){l.push(a)});l.sort(function(a,b){return a.p-b.p});a.srPages=l},moveUp:function(a){d.selectedSuggestion--; -1>=d.selectedSuggestion&&(d.selectedSuggestion=-1);if(-1==d.selectedSuggestion)a.searchForm.q=d.userSearchInput,a.selectedSuggestion=null,a.suggestedFeature=
null;else{var e=a.searchResultsPage.poi_class.slice(0),e=e.concat(a.searchResultsPage.features);a.searchForm.q.split(/[\s,;.]+/);var e=e[d.selectedSuggestion],f=d.getToken(e),k=" "===d.queryHead.slice(-1)?"":" ",f=d.queryHead+k+f,f=" "===f.slice(-1)?f:f+" ";d.suppressOnSearchQuerry=!0;a.searchForm.q=f;a.selectedSuggestion=d.getId(e);a.suggestedFeature=e}},moveDown:function(a){var e=a.searchForm.q.split(/[\s,;.]+/);-1==d.selectedSuggestion&&(d.userSearchInput=a.searchForm.q,e.pop(),d.queryHead=
e.join(" "));e=a.searchResultsPage.poi_class.slice(0);e=e.concat(a.searchResultsPage.features);d.selectedSuggestion++;d.selectedSuggestion>=e.length&&(d.selectedSuggestion=e.length-1);var e=e[d.selectedSuggestion],f=d.getToken(e),k=" "===d.queryHead.slice(-1)?"":" ",f=d.queryHead+k+f,f=" "===f.slice(-1)?f:f+" ";d.suppressOnSearchQuerry=!0;a.searchForm.q=f;a.selectedSuggestion=d.getId(e);a.suggestedFeature=e},notNullPoiFilters:function(a){a=a.activePoiFilters;if(!a)return null;for(var d={},f=Object.keys(a),
e=0;e<f.length;e++){var h=f[e];if(angular.isObject(a[h])){for(var m=a[h],l=Object.keys(m),p=[],u=0;u<l.length;u++){var q=l[u];m[q]&&p.push(q)}p&&0<p.length&&(d[h]=p)}else a[h]&&(d[h]=!0)}return 0==Object.keys(d)?null:d},getToken:function(a){return a.type?a.name||a.housenumber:a.translated_title[0]},getId:function(a){return a.id||a.name}};return d}]);var meOSMDoc=angular.module("meOSMDoc",["meMap"]);
...
function(d,e){d.selected?g.addSelection.apply(g,[a,d,e]):g.removeSelection.apply(g,[a,d,e]);var h=g.expandCathegories.apply(g,[a]);b.filterMarkersByTypes.apply(b,[a,h])};a.listMoreTags=function(b){if(!b||!a.name2FClass)return null;if(b.__translatedTags&&0<b.__translatedTags.length)return b.__translatedTags;var d=b.poi_class,e=[];angular.forEach(b.more_tags,function(b,g){if("description"!=g)for(var k in d){var h={},n=a.name2FClass[d[k]];n&&(n=n.more_tags[g])&&(h.name=n.name,h.type=n.valueType,h.key=
g,n.values[b]?(h.value=n.values[b].name,h.group=n.values[b].group):h.value=b,e.push(h))}});return b.__translatedTags=e}},selectPoiType:function(a,b){this.addSelection(a,a.name2FClass[b],"features")},loadTree:function(b,e,g){a.get(API_ROOT+"/osmdoc/hierarchy/"+e+"/"+g).success(function(a){b.hierarchy=a;h(b.hierarchy,{feature:function(a){b.name2FClass[a.name]=a},group:function(a){b.name2Group[a.name]=a}},[]);b.$broadcast("HierarchyLoaded")})},expandCathegories:function(a){if(!a.hierarchy)return[];var b=
...
filterAndSort(g.cathegories.features)},loadTagValuesStatistic:function(b){if(USE_POI_TAGS_FILTERS){var e=this.cathegories.features,g=this.cathegories.groups;0<e.length||0<g.length?a.get(API_ROOT+"/osmdoc/statistic/tagvalues.json",{params:{poiclass:e,poigroup:g,lang:b.lng,hierarchy:b.hierarchyCode}}).success(function(a){var d={_ordered_keys:[]},e=a.tagValuesStatistic;a=a.tag_options.commonTagOptions;d.opening_hours={key:"opening_hours",type:"WORKING_HOURS",title:b.translation["details.poi.tag.values.wh.24_7"]};
d._ordered_keys.push("opening_hours");for(var g={opening_hours:{"24_7":!1}},h=0;h<a.length;h++){var k=a[h];if("GROUP_TRAIT"==k.type){d._ordered_keys.push(k.key);var r=[],v;for(v in k.options){var t=k.options[v],y=e[t.valueKey];y&&(g[t.valueKey]=!1,r.push(t))}d[k.key]={key:k.key,title:k.title,type:k.type,options:r}}else if(e[k.key]&&"BOOLEAN"==k.type)d._ordered_keys.push(k.key),g[k.key]=!1,d[k.key]=k;else if(e[k.key]&&"ENUM"==k.type){d._ordered_keys.push(k.key);r=[];g[k.key]={};for(v in k.options)if(t=
k.options[v],y=e[k.key][t.valueKey])g[k.key][t.valueKey]=!1,r.push(t);d[k.key]={key:k.key,title:k.title,type:k.type,options:r}}else if(e[k.key]&&"ANY_STRING"==k.type){d._ordered_keys.push(k.key);g[k.key]={};var r=[],w;for(w in e[k.key])g[k.key][w]=!1,r.push({valueKey:w,valueTitle:w});d[k.key]={key:k.key,title:k.title,type:k.type,options:r}}}console.log(d);b.poi_subfilters=d;b.activePoiFilters=g}):b.poi_subfilters=null}}};return g}])})(angular,meOSMDoc);var meDetails=angular.module("meDetails",["meMap","meI18n"]);
...
d[0]||"bus_stop"==d[0])&&null==g.ptCache.get(b)&&(a=b.split("-")[2])){d="";if("n"==a.charAt(0))d+="node";else if("w"==a.charAt(0))d+="way";else if("r"==a.charAt(0))d+="rel";else return;d+="("+a.substring(1)+");";e.post("http://overpass-api.de/api/interpreter","data="+("[out:json];("+d+");<<; out tags qt;")).success(function(a){if(a.elements){for(var d={},e=0;e<a.elements.length;e++){var f=a.elements[e];if("relation"==f.type&&"route"==f.tags.type){var q=f.tags.route;!q||"bus"!=q&&"tram"!=q||(null==
d[q]&&(d[q]=[]),d[q].push(f.tags.ref))}}g.ptCache.put(b,{id:b,routes:d});h.$$phase||h.$apply()}})}},_load:function(a,b,h,m){var l=API_ROOT+"/location/"+b;h&&(l+="/_related");e.get(l).success(function(e){function h(a,b){d.ready?l(a,b):window.setTimeout(function(){h(a,b)},500)}function l(a,d){g.cache.put.apply(g.cache,[b,d]);m(d)}e&&e.id&&h(a,e)})}};return g}]);FixedSizeFIFOCache=function(e){this.size=e||20;this.storage={};this.queue={};this.pointer=0};
