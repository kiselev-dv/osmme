...
d.coords.latitude,f=d.coords.longitude;500<l.distanceTo([e,f])&&r.sendRequest(f,e,!1,function(d){d&&d.text?(d.lat=e,d.lon=f,b.browserGeoLocation=d):b.browserGeoLocation&&delete b.browserGeoLocation})}})});b.activeFeatureID=p.fid;b.explain=p.explain;b.content=p.details?"details":"map";b.activeFeatureID&&("details"==b.content?m.showDetails(b,b.activeFeatureID):m.showPopup(b,b.activeFeatureID));b.$on("$locationChangeSuccess",function(){var e=b.activeFeatureID,f=l.getParameters();b.lng!=f.lang&&(h.lang=
f.lang,l.reload());b.activeFeatureID=f.id;b.explain=f.explain;b.content=f.details?"details":"map";"details"==b.content?(ANALYTICS_CODE&&q.trackPage("/#!/"+b.lng+"/id/"+b.activeFeatureID+"/details"),m.showDetails(b,b.activeFeatureID),e&&m.closePopup(b,e)):(b.activeFeatureID?(ANALYTICS_CODE&&q.trackPage("/#!/"+b.lng+"/id/"+b.activeFeatureID),m.showPopup(b,b.activeFeatureID)):void 0!==e&&e!=b.activeFeatureID&&m.closePopup(b,e),l.getParameters().map&&(e=l.getParameters().map,d.broadcastAction||d.setView(e[1],
e[2],e[0])));f.strict&&(b.strictSearch=!0);f.q&&null==b.searchQuerry&&b.searchQuerry!=f.q&&(b.searchQuerry=f.q,b.$broadcast("Search",f.q));v()});b.$on("Search",function(){l.update("q",b.searchQuerry)});b.$on("SelectFeature",function(){l.update("q",b.searchQuerry)});b.$on("HierarchyLoaded",v);b.$on("PopupClose",function(d,e){"map"==b.content&&l.update("id",null)});b.$on("PopupOpen",function(b,d){l.update({id:d,map:null})});b.$on("MapViewChanged",function(){l.update("map",d.getStateArray())});n=function(){b.osmdocCat&&
(k.organizeCathegories(),0<b.osmdocCat.features.length?l.update("pt",b.osmdocCat.features.join()):l.update("pt",null),0<b.osmdocCat.groups.length?l.update("pg",b.osmdocCat.groups.join()):l.update("pg",null),d.filterMarkersByTypes(b,k.expandCathegories(b)),k.updateSelections(b))};b.$on("CloseSearchResults",function(){b.searchQuerry="";b.searchResultsPage=null;b.srPages=null;d.filterMarkersByTypes(b,k.expandCathegories(b));k.updateSelections(b)});b.$on("SelectCathegoryTreeNode",n);b.$on("UnselectCathegoryTreeNode",
n);b.formatObjectType=function(d){if(d){if(d.poi_class){var f=b.translateTypeNames(d);if(0<f.length)return f.join(", ")}if(d.weight_base_type)return e.tr(b,"weight_base_type."+d.weight_base_type)}return null};b.translateTypeNames=function(d){var f=[];if(d&&d.poi_class)for(var e in d.poi_class){var k=d.poi_class[e];b.name2FClass&&b.name2FClass[k]&&(k=b.name2FClass[k].translated_title,k.toUpperCase()!==(d.name||"").toUpperCase()&&f.push(k))}return f};b.nameContainsType=function(d){if(d){var f=b.translateTypeNames(d);
d=(d.name||"").toLowerCase();for(var e in f)if(0<=d.indexOf(f[e].toLowerCase()))return!0}return!1};b.tagValueHTML=tagValueHTML;b.buildingType=function(d){return e.tr(b,"details.adr.building")};b.mergeIntoPath=function(d,b){return"/#!"+l.mergeIntoPath(d,b)};b.searchKeyDown=function(d){38==d.keyCode?(d.stopPropagation(),d.preventDefault(),b.$broadcast("SearchKeyUp")):40==d.keyCode&&b.$broadcast("SearchKeyDown")};b.$watch("searchQuerry",function(d,f){""!=f&&""==d&&b.$broadcast("CleanSearchInput")});
b.$on("CleanSearchInput",function(){l.update("q",null)});b.formatSearchResultTitle=function(d){if(!d)return"";if("adrpnt"==d.type)return e.tr(b,"map.js.search.title.adrpnt");if(d.name||d.poi_class){var f=b.formatObjectType(d),k=d.name||f;d.name&&f&&(k+=" ("+f+")");return k}return""};b.formatSearchResultAddress=function(d){return d?getAddress(d,b.translation?b.translation["addr.order"]:"hn-street-city")[0]:""};b.getAddress=function(d){return getAddress(d,b.translation?b.translation["addr.order"]:"hn-street-city")};
b.selectRow=function(d){b.$broadcast("SelectFeature",d)};b.searchInputEnter=function(){b.suggestedFeature?b.suggestedFeature.feature_id?b.$broadcast("SelectFeature",b.suggestedFeature):b.selectPoiType(b.suggestedFeature.name):b.$broadcast("Search",b.searchQuerry)};b.removeSelected=function(d,f){k.removeSelection(b,{name:d},f)};b.moveToBrowserGeoLocation=function(b){d.setView(b.lat,b.lon,14);d.saveStateToCookie()}}]);
