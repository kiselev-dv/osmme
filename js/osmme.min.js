...
b.lng=p.lang||initLang(h);b.hierarchyCode=k.getHierarchyCode(b.lng);b.name2FClass={};b.name2Group={};k.loadTree(b,b.lng,b.hierarchyCode);e.getTranslation(b,b.lng,!0,function(){var g=[null,null,null];p.map&&(g=p.map);var l=d.createMap(b,g[1],g[2],g[0]);r.create(b,l);k.attach(b);f.attach(b);b.dayNames=e.tr(b,"details.poi.tag.values.wh.days").split(" ");d.enquirePosition(function(d){if(d){var e=d.coords.latitude,f=d.coords.longitude;500<l.getCenter().distanceTo([e,f])&&r.sendRequest(f,e,!1,function(d){d&&
...
this.parameters[k],r=b[f];if("anonymous"==m.type){d[m.name]=r;e++;break}else if("flag"==m.type&&m.name==r){d[m.name]=!0;e++;break}else if("parameter"==m.type&&m.name==r){if(m.slashes)for(k=f+1;k<Math.min(b.length,f+1+m.slashes);k++)d[m.name]||(d[m.name]=[]),d[m.name].push(b[k]);else d[m.name]=b[f+1];f+=m.slashes||1;e++;break}e++}return d};b.prototype.createPath=function(b){for(var e="/",d=0;d<this.parameters.length;d++){var f=this.parameters[d];"anonymous"==f.type&&b[f.name]&&(e+=b[f.name]+"/");"flag"==
...
b.srPages=l},moveUp:function(b){e.selectedSuggestion--; -1>=e.selectedSuggestion&&(e.selectedSuggestion=-1);if(-1==e.selectedSuggestion)b.searchForm.q=e.userSearchInput,b.selectedSuggestion=null,b.suggestedFeature=null;else{var f=b.searchResultsPage.matched_type.slice(0),f=f.concat(b.searchResultsPage.features);b.searchForm.q.split(/[\s,;.]+/);var f=f[e.selectedSuggestion],g=e.getToken(f),h=" "===e.queryHead.slice(-1)?"":" ",g=e.queryHead+h+g,g=" "===g.slice(-1)?g:g+" ";e.suppressOnSearchQuerry=!0;
b.searchForm.q=g;b.selectedSuggestion=e.getId(f);b.suggestedFeature=f}},moveDown:function(b){var f=b.searchForm.q.split(/[\s,;.]+/);-1==e.selectedSuggestion&&(e.userSearchInput=b.searchForm.q,f.pop(),e.queryHead=f.join(" "));f=b.searchResultsPage.matched_type.slice(0);f=f.concat(b.searchResultsPage.features);e.selectedSuggestion++;e.selectedSuggestion>=f.length&&(e.selectedSuggestion=f.length-1);var f=f[e.selectedSuggestion],g=e.getToken(f),h=" "===e.queryHead.slice(-1)?"":" ",g=e.queryHead+h+g,g=
" "===g.slice(-1)?g:g+" ";e.suppressOnSearchQuerry=!0;b.searchForm.q=g;b.selectedSuggestion=e.getId(f);b.suggestedFeature=f},getToken:function(b){return b.type?b.name||b.housenumber:b.translated_title[0]},getId:function(b){return b.feature_id||b.name}};return e}]);var meOSMDoc=angular.module("meOSMDoc",["meMap"]);
...
function(b){d[b]=1});var e=this;h(b.hierarchy,{feature:function(b,f){g.forEach(e.cathegories.groups,function(e){0<=f.indexOf(e)&&(d[b.name]=1)})}},[]);var f=[];g.forEach(d,function(b,d){f.push(d)});return f},addSelection:function(b,d,e){e=this.cathegories[e];d&&e&&e.push(d.name);b.$broadcast("SelectCathegoryTreeNode")},removeSelection:function(b,d,e){e=this.cathegories[e];if(d&&e)for(var f=0;f<e.length;f++)if(e[f]==d.name){e.splice(f,1);break}b.$broadcast("UnselectCathegoryTreeNode")},updateSelections:function(b){h(b.hierarchy,
